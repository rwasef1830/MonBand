...
public abstract class PollingTrafficRateServiceBase : ITrafficRateService
{
    ...
    readonly object _startStopLocker = new();
    bool _disposed;
    Task? _pollLoop;
    ...
    async Task DoPollLoop()
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                this.Log.LogTrace("Poll time duration: {PollTimeElapsed}", pollElapsed);
            ...
            {
                this._pollStopwatch.Restart();
                this.Log.LogTrace("Sleeping for {DelayInterval}", delayInterval);
